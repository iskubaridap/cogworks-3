...
import { setCanvasFileElementClicked, getAllowedElements } from '../utils/global-value/elements.js';
...
export default class File {
    constructor(element, id, callback) {
        ...
        this.setUndoRedo = (_name, _undo, _redo) => {
            const _this = this;
        ...
        element.addEventListener('load', function (e) {
            ...
            _this.iframeElem = e.target;
            _this.contentWin = _this.iframeElem.contentWindow;
    ...
    removeAllContentEditableElements() {
        ...
        const elemCollection = win.document.body.getElementsByTagName('*');
        ...
        for (const elem of elemCollection) {
            getAllowedElements().forEach(value => {
                if ((elem.tagName).toLowerCase() == value) {
                    ...
                    if (elem.contentEditable !== undefined) {
    ...
    getElementSelected() {
        ...
        let elemEvtHover = null;
        let elemHover = null;
        let elemDragOver = null;
        let elemDbClick = null;
        ...
        let offset = null;
        let pointerPosition;
        const hideOnHoverEffect = (_elem) => {
            if (_elem.classList.contains('invalid'))
                _elem.classList.remove('invalid');
            if (_elem !== elemSelected)
                _elem.classList.remove('elem-on-hover');
        };
        const showOnHoverEffect = (_elem) => {
            _elem.classList.add('elem-on-hover');
        };
        const getValidParent = (_elem, parent, action) => {
        ...
        const hideHoverElement = (evt) => {
            if (elemHover !== null) {
                tagName.classList.remove('show');
                tagName.classList.remove('drag-over');
                if (tagName.classList.contains('invalid'))
                    tagName.classList.remove('invalid');
                hideOnHoverEffect(elemHover);
                _this.removeOnHoverArrowEffect();
            }
        };
        const selectElemEvent = (evt) => {
            evt.preventDefault();
        ...
        const setElemToMoveEvent = (evt) => {
            evt.preventDefault();
            if (elemToMove == null) {
                elemToMove = evt.target;
        ...
        const elemOnDragEvent = (evt) => {
            evt.preventDefault();
            let vPElem;
            let foundValidParent;
            if (elemToMove !== null) {
                ...
                if (_this.contentWin.document.caretPositionFromPoint) {
                    range = _this.contentWin.document.caretPositionFromPoint(evt.clientX, evt.clientY);
                    textNode = range.offsetNode;
                    offset = range.offset;
                }
                else if (_this.contentWin.document.caretRangeFromPoint) {
                    range = _this.contentWin.document.caretRangeFromPoint(evt.clientX, evt.clientY);
        ...
        const unsetElemToMoveEvent = (evt) => {
            evt.preventDefault();
            ...
            if (elemToMove != null && elem !== elemToMove) {
                if (pointerPosition !== 'center') {
                    elemParent = evt.target.parentNode;
                }
                else {
                    elemParent = evt.target;
                }
        ...
        const setElemContentEditable = (evt) => {
            elemSelected = evt.target;
            elemDbClick = evt.target;
